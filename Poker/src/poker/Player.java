...
public class Player {
	...
	//change illusions of fold or raise removal to actual removals
	//add feature to allow players who do not have enough money to call a bet call anyway by going all in (WIP)
	...
	public void takeTurn() throws InputMismatchException { //performs player actions for player object following input
		...
		for(int i=0;describedTurns<totalTurnsTaken-1;i++) { //display all turns before our own turn
			...
			if(playerAction.startsWith("check")) {
				...
				if(Poker.playerList[i].getMoney()>0) {	
					System.out.println(actingName+" called at "+playerAction.substring(4));
				} else {
					System.out.println(actingName+" went all in to call!");
				}
				...
				if(Poker.playerList[i].getMoney()>0) {
					...
					System.out.println(actingName+" went all in!");
				}
		...
		} else if(!Poker.findAllInPlayers()) {
		...
		if(response.equals("a")&&currentBet==Poker.highBet) { //if response was a and your option was check, run check code
			...
			//start of call code=============================================================================================
		} else if(response.equals("a")&&currentBet!=Poker.highBet&&!Poker.findAllInPlayers()) { //normal call detection
			...
			//start of all in call code======================================================================================
		} else if(response.equals("a")&&Poker.findAllInPlayers()) { //WIP
			...
			if(intResponse<Poker.blind&&money>=Poker.blind) { //in case of silly billys
				System.out.println("Invalid bet. Your bet must be >= the big blind");
				turnsTaken--; //decrement turnsTaken so this turn isn't counted twice
				takeTurn(); //try again
				return; //do nothing else this instance of the method
			} else if(intResponse>money) {
				System.out.println("Invalid bet. You cannont bet more than you have");				
				turnsTaken--; //decrement turnsTaken so this turn isn't counted twice
				takeTurn(); //return
				return; //do nothing else this instance of the method
			}
			...
			if(betMoney) { //if you bet then it records a bet and the ammount you bet
